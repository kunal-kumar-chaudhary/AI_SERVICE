...
async def get_embedding(text: str) -> List[float]:
    """Get embedding for a single text using aiohttp"""
...
async def get_embeddings_batch(texts: List[str], max_workers: int = 5) -> List[List[float]]:
    """
...
def get_embeddings_batch_sync(texts: List[str], max_workers: int = 5) -> List[List[float]]:
    ...
    return asyncio.run(get_embeddings_batch(texts, max_workers))